...
DO NOT USE THIS FILE IF YOU ARE USING THE LIBRARY IN ANY WAY. 
...
N scripts/scrape_categories.py\n\"\"\"\n\n"

def scrape_categories():
    p = requests.get(ARXIV_CATEGORIES_PAGE)
    content = html.fromstring(p.content)
    categories_table_caption = content.xpath('.//div/table/caption[text()="Table: Subject Classifications"]')
    assert len(categories_table_caption) == 1, "Invalid length of categories table"
...
def create_category_enum(categories):
    ...
    for i, e in enumerate(categories):
        ...
        enum_file += "    %s = %i  # %s\n" % (escaped_name,
                                              i + 1, 
                                              e[1])
        category_map_dict_items.append("    ArxivCategory.%s: '%s'" % (escaped_name, e[0]))
